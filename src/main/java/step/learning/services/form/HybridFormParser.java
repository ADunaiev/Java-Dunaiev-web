...
public class HybridFormParser implements  FormParseService{
    ...
    private  static  final  int FILE_LIMIT = 20 *1024 *1024; // 10 MB
    private  static  final  int FORM_LIMIT = 30 *1024 *1024; // 10 MB
    ...
    public HybridFormParser() {
        ...
        fileUpload.setFileCountMax( FILE_LIMIT );
    ...
    public FormParseResult parse(HttpServletRequest request) {
        ...
        if(isMultipart){ // форма з файлами та полямию Користуємось Apache
            try {
                for (FileItem item : fileUpload.parseRequest(request)) {
                    if (item.isFormField()) { // просто поле
